...
pub fn get_banner(bannered: String) -> String {
	...
	if !Path::new(&config_file).exists(){
		...
		if fs::create_dir_all(format!("{}/.config/fetrust", env!("HOME"))).is_err() {
			println!("Error: Something happened wrong (creating {})", format!("{}/.config/", env!("HOME")))}
...
fn main() {
	...
	for info in ["banner","user_a_host_name","brace","os","kernel","shell","family","uptime","cpu_type"] {
		match json.as_ref().unwrap().get(info) { Some(info) => {
			match info {
				Json::OBJECT { name, value } => {
					let mut printing = "".to_string();
					let mut printingc = "".to_string();
					match value.unbox() { Json::ARRAY(value) => {
						match value[0].unbox() { Json::ARRAY(value) => {
						...
						match value[1].unbox() { Json::STRING(value) => {
							match value.as_str() {
								"black" => {
